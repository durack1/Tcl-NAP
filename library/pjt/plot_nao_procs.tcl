...
# $Id: plot_nao_procs.tcl,v 1.104 2004/09/27 04:43:56 dav480 Exp $
#
...
namespace eval Plot_nao {
    ...
    variable ox "";		# "$x $y $z" (for user convenience)
    variable oy "";		# "$x $y $z" (for user convenience)
	...
	namespace eval ${window_id} {
	    ...
	    variable parent "";	        # Parent window ID
	...
	set buttonCommand "lappend Plot_nao::$window_id\::save \$Plot_nao::xyz; \
        set Plot_nao::ox \$Plot_nao::x; set Plot_nao::oy \$Plot_nao::y"
...
#	set parent ""
	...
	set width "";			# width (in pixels) of image (can be "min max" for image)
        #
        # Corrected for names including spaces
        # PJT 2004
        set shp [$nao shape] 
        set dindx 0
        foreach dim $shp {
            lappend dim_names [$nao dim $dindx]
            incr dindx
        }
	...
	set call_level "#[expr [info level]-3]"; # stack level from which plot_nao called
	...
	if {$nplots > 1} {
		...
		t* {
		    ...
		    Plot_nao::draw_tiles   $all $window_id $graph $nao $title \
			    $ncols $sub_title $range_nao $want_yflip $major_tick_length
		}
		x* {
		    set plot_type xy
		    Plot_nao::draw_xy $all $window_id $graph $nao $title $range_nao $height $width \
	...
	append command " -geometry NW -parent $all -text \$Plot_nao::${window_id}::new_width]"
	...
	append command " -geometry NW -parent $all -text \$Plot_nao::${window_id}::new_height]"
	append command "; set Plot_nao::${window_id}::magnification {}"
	append command "; $cond_redraw_command"
	...
	append command " -geometry NW -parent $all -text \$Plot_nao::${window_id}::key_width]"
	append command "; $cond_redraw_command"
	...
	append command " -geometry NW -parent $all -text \$Plot_nao::${window_id}::gap_width]"
	append command "; $cond_redraw_command"
	...
	append command " -geometry NW -parent $all -text \$Plot_nao::${window_id}::gap_height]"
	append command "; $cond_redraw_command"
	...
	set filename [tk_getSaveFile -initialfile $filename -title "Image Filename" -parent $top]
	destroy $top
	if {$filename ne ""} {
	    raise $all
	    update
	...
	if {$have_key} {
	    ...
	    draw_key $window_id $graph $x $margin_top $new_height $major_tick_length
	}
	...
	bind $graph <B1-Motion> "Plot_nao::box_xyz \
		[list $all $window_id $graph $margin_left $margin_top %x %y]"
	bind $graph <ButtonRelease-1> "Plot_nao::buttonup_xyz \
		[list $all $window_id $graph $margin_left $margin_top %x %y]"
	raise $all
	update
    }
	...
	if {$discrete_colors} {
	    ...
	    nap "j = 0 .. (nels(i) - 2)"
	    nap "count = (i(j+1) - i(j))(-)"
	    nap "imid  = (i(j+1) + i(j))/2 <<< 254"
	    nap "mapping = count # imid // 255"
	...
	set xkey [expr $x + $key_width + 2]
	draw_y_axis $window_id $graph "" $xkey $y $nrows $cvz 1 $major_ticks_z \
	...
	nap "max_n = i32(nint(min(abs(i(j+1) - i(j))) / min_interval))"
	...
	$cv set unit [fix_unit [$major_ticks unit]]
    ...
    #
    # there is a large overhead in repeated code in these routines
    # the prospect of cleaning this up looks a bit tricky so leave it
    # alone because it seems to work!
    #
    # box_xyz --
    #
    # called when pointer moves within z-plot (image plot)
	...
	if {![winfo exists $all.xyz]} {
        ...
        #
        # Convert ox and oy back to screen coordinates
	nap "cvx = coordinate_variable(image_nao,-1)"
	nap "cvy = coordinate_variable(image_nao,-2)"
        nap i = cvx@@ox + $margin_left
        nap j = cvy@@oy + $margin_top
	set scroll_region [$graph cget -scrollregion]
	set width_total  [lindex $scroll_region 2]
	set height_total [lindex $scroll_region 3]
	set can_x [$graph canvasx $screen_x]
	set can_y [$graph canvasy $screen_y]
        # Make sure that the point is on the
        # image!
	nap "nx = nels(cvx)"
	nap "ny = nels(cvy)"
        nap "mx = nx + $margin_left - 1"
        nap "my = ny + $margin_top - 1"
        nap canx = (can_x <<< mx) >>> $margin_left
        nap cany = (can_y <<< my) >>> $margin_top
	$graph delete box
	$graph create rect [$i] [$j] [$canx] [$cany] -tag box
	nap "i = can_y - $margin_top  >>> 0 <<< ny - 1"
	nap "j = can_x - $margin_left >>> 0"
	set x [[nap "cvx(j <<< nx - 1)"] -format %0.6g]
	set y [[nap "cvy(i)"] -format %0.6g]
    ...
    # buttonup_xyz --
    #
    # called when the button is released (image plot)
	...
	global Plot_nao::${window_id}::gap_width
	global Plot_nao::${window_id}::image_nao
	global Plot_nao::${window_id}::parent
	if {![winfo exists $all.xyz]} {
	    Plot_nao::create_xyz $all $window_id $graph 1
	    $graph config -cursor crosshair
	}
	...
	nap cany = (can_y <<< my) >>> $margin_top
        #
        # If the button was released at the same place it
        # was pressed we want to clear any boxes!
        set write_result 1
        if {[$i] == [$canx] && [$j] == [$cany]} {
            $graph delete box
            set write_result 0
...
#	$graph create rect [$i] [$j] [$canx] [$cany] -tag box
	...
	if {[$image_nao rank] == 3} {
        ...
        #
        # We now want to change the area_select value in hdf if applicable
        set pname [string range $parent 1 3]
        #
        if {$write_result && ($pname == "hdf" || $pname == "net")} {
            set pname [string range $parent 1 end]
	    set namex  [$image_nao dim -1]
	    set namey  [$image_nao dim -2]
            # Probably should check for existence of the variable before
            # setting it! 
            set ::Hdf::t_${pname}::area_select [list $namex $ox $x $namey $oy $y] 
        }
    ...
    # May also set want_equalise, upper
    #
	...
	set redraw_command [list Plot_nao::redraw_tiles $all $window_id $graph $nao \
		$nlevels $ny $nx $nao2d $title $ncols $sub_title $range_nao $major_tick_length]
	...
	set ncols [layout_tiles $window_id $nlevels $ncols $screen_height $screen_width \
		...
		$major_tick_length $axis_text_width]
	nap "j = 0 // (nx - 1)"
	...
	if {$key_width > 0} {
	    ...
	    draw_key $window_id $graph $x $title_height $key_height $major_tick_length
	}
