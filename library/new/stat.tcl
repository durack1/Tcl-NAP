...
# $Id: stat.tcl,v 1.22 2005/02/10 05:51:07 dav480 Exp $
#
...
namespace eval ::NAP {
    ...
    # nc = number of class intervals (default: 64)
	...
	if {$vr == $r} {
	    nap "pc = reshape(pc)"; # ensure vector
	    set np [$pc nels]
	    set com [commas "$r-1"]
	    nap "result = X(1 .. np$com)"
	    eval $result set dim [lreplace [$X dim] 0 0 percentile]
	    eval $result set coo [lreplace [$X coo] 0 0 pc]
	    nap "p = 0.01 * pc"; # convert pc to probability
	    nap "xmin = min X"
	    nap "xmax = max X"
	    nap "c = (xmax - xmin) / nc"
	    nap "ix = i32(floor((X - xmin) / c)) >>> 0 <<< (nc - 1)"
	    nap "f = 0 // (# ix)"
	    nap "cf = reshape(0, shape f)"; # cumulative frequencies
	    for {set i 1} {$i <= $nc} {incr i} {
		$cf set value "cf(i-1$com) + f(i$com)" "i$com"
	    }
	    nap "rcf = f64(cf) / cf(-1$com)"; # relative cumulative frequencies
	    unset f cf
	    for {set i 0} {$i < $np} {incr i} {
		$result set value "xmin + c * (rcf @ p(i))" "i$com"
	    }
	} else {
	    nap "d = r - vr"
	    nap "perm = 0 .. (r-1)"
