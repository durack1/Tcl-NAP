...
# $Id: plot_nao_procs.tcl,v 1.131 2006/02/10 06:25:15 dav480 Exp $
#
...
namespace eval Plot_nao {
	...
	switch [string toupper [string index $overlay_option 0]] {
		...
		if [catch "uplevel $call_level nap \
			\"::Plot_nao::${window_id}::overlay_nao = ($expr)(@@$y,@@$x)\"" result] {
		    handle_error $result
		    return
		}
	    }
	...
	puts "major_ticks_x=$major_ticks_x"
	if {$major_ticks_x eq ""} {
	puts "cvx=[$cvx]"
	    nap "major_ticks_x = axis_major_ticks_span(cvx, 32)"
	puts "a major_ticks_x=[$major_ticks_x a]"
	...
	puts "b major_ticks_x=[$major_ticks_x a]"
	} else {
	...
	puts "major_ticks_x=[$major_ticks_x]"
	puts "new_width=$new_width xend=$xend x0=$x0"
	nap "xslope = new_width / f64(xend - x0)"; # slope for x mapping
	set width_bar [[nap "xslope * barwidth"]]
	puts "xslope=[$xslope] barwidth=$barwidth width_total=$width_total width_bar=$width_bar"
	...
	if {[$major_ticks step] == "AP"} {
	    ...
	    nap "to   = major_ticks(0) - <((major_ticks(0) - cv(-1)) / step) * step"
