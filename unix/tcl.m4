...
AC_DEFUN(SC_CONFIG_CFLAGS, [
	...
	*win32*|*WIN32*|CYGWIN_NT*|cygwin_nt*)
	    ...
	    LDFLAGS_OPTIMIZE="-nodefaultlib -release"
	    LIB_PREFIX=""
	    EXTRA_CFLAGS=""
	    PATHTYPE=-w
	    STLIB_LD="lib -out:"
	...
	IRIX-4.*)
	    ...
	    ;;
	IRIX-5.*|IRIX-6.*|IRIX64-6.4*|IRIX64-6.5*)
	    SHLIB_CFLAGS=""
	...
	[tcl_ok=$enableval], [tcl_ok=yes])
    AC_MSG_CHECKING(for build with dynamic loading (so define USE_TCL_STUBS))
    AC_MSG_RESULT($tcl_ok)
    if test "$tcl_ok" = "yes"; then
