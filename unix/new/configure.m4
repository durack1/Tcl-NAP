...
do
	...
	do
	    ...
	    if test -r $DIR/libnc-dodsMT.lib; then
		NC_LIB_DIR=$DIR
	    fi
...
case "$HOST_OS" in
    *win32* | *WIN32* | *CYGWIN_NT*)
	...
	if test -r $NC_LIB_PATH; then
	    ...
	    NC_LIB_PATH="$NC_LIB_DIR/libnc-dodsMT.lib"
	    NC_LIB_SPEC=''
	    for TMP in nc-dods dap nc-dods dap curl xml2
	    do
		NC_LIB_SPEC="$NC_LIB_SPEC'`cygpath -w $NC_LIB_DIR/lib${TMP}MT.lib`' "
		echo "NC_LIB_SPEC=;$NC_LIB_SPEC;"
	    done
	    NC_LIB_DIR="`cygpath -w $NC_LIB_DIR`"
	fi
	SHLIB_DIR="\${bindir}"
