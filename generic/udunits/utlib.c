/*
 * $Id: utlib.c,v 1.6 2005/11/07 05:33:15 dav480 Exp $
 *
...
#include "utprivate.h"
    ...
    #define UT_DEFAULT_PATH "udunits.dat"
#endif
    ...
    utUnit		*unit;
{
    ...
    result = unit->hasorigin == 0  &&  unit->origin == 0.0  &&  unit->factor == 1.0;
    for (iquan = 0; iquan < UT_MAXNUM_BASE_QUANTITIES; ++iquan)
	result = result  &&  unit->power[iquan] == 0;
    return result;
}
            ...
            for (iquan = 0; iquan < UT_MAXNUM_BASE_QUANTITIES; ++iquan) {
                if (unit->power[iquan] != 0) {
                    if (unit->power[iquan] == 1) {
                        ...
                        (void)sprintf(buf, "%s%.0f ", BaseName[iquan],
                                      unit->power[iquan]);
                    }
                    buf += strlen(buf);
                }
    ...
    int		n)
{
	...
	"I/O error",
	"Invalid unit-structure",
	"Package not initialized",
	"Two units are not convertable",
	"Memory allocation failure",
	"Insufficient room supplied",
	"Not a unit of time",
	"Duplicate unit"
    };

    return str[n > -12  &&  n < 0 ? -n : 0];
}
