...
#line 5 "/home/dav480/tcl/nap/generic/nap_ooc.c,m4"
    ...
    char                *text)          /* result */
{
    ...
    if (IsNaN64(value)) {
	str = strcpy(text, "NaN");
	CHECK2(str, "Nap_F64ToText: Error calling strcpy");
	...
	assert(format);
	str = strchr(format, '%');
	CHECK2(str, "Nap_F64ToText: No '%' in format");
	str = strpbrk(str, "dioxXucfEegG");
	CHECK2(str, "Nap_F64ToText: Illegal format");
	switch (tolower(*str)) {
	case 'e':
	case 'f':
	case 'g':
	    status = sprintf(text, format, value);
	    CHECK2(status >= 0, "Nap_F64ToText: Error calling sprintf");
	    str = strpbrk(text, "Ee");
    ...
    char                *s)
{
    ...
    CHECK2(dataTypeText, "Nap_AppendNAOHeader: error calling Nap_DataTypeToText");
    ...
    status = sprintf(s,
	    ...
	    null_str,
	    naoPtr->count - 1);
    CHECK2(status > 0, "Nap_AppendNAOHeader: error calling sprintf");
    FREE(dataTypeText);
    if (naoPtr->unit) {
    ...
    int			*keep)			/* -keep specified? (out) */
{
	...
	for (i = 0; i < objc; ++i) {
		...
		case 1:			/* columns */
		    ++i;
		    CHECK2(i < objc, "Nap_AppendNAOValue: -columns without value");
		...
		case 2:			/* format */
		    ++i;
		    CHECK2(i < objc, "Nap_AppendNAOValue: -format without value");
		    formatPtr = Tcl_GetStringFromObj(objv[i], NULL);
		    CHECK2(formatPtr, "Nap_AppendNAOValue: Error calling Tcl_GetStringFromObj");
		    if (formatPtr[0]) {
		...
		case 3:			/* lines */
		    ++i;
		    CHECK2(i < objc, "Nap_AppendNAOValue: -lines without value");
		    status = Tcl_GetIntFromObj(nap_cd->interp, objv[i], &j);
		    CHECK(status == TCL_OK);
		    maxLines = j;
		    break;
		case 4:			/* missing */
		    ++i;
		    CHECK2(i < objc, "Nap_AppendNAOValue: -missing without value");
		    missingValueText = Tcl_GetStringFromObj(objv[i], NULL);
		    CHECK2(missingValueText, "Nap_AppendNAOValue: Error calling Tcl_GetStringFromObj");
		    break;
		case 5:			/* keep */
		    *keep = 1;
		    break;
	...
	case NAP_RAGGED:
	    ...
	    CHECK2(startNao, "Nap_AppendNAOValue: Error calling Nap_GetNaoFromSlot");
	    CHECK2(startNao->dataType == NAP_I32,
		    "Nap_AppendNAOValue: Ragged start NAO has wrong data type");
	    for (j = 0; j < naoPtr->nels; j++) {
		status = sprintf(text, "%d   start-index: %d  ",
			j, startNao->data.I32[j]);
		CHECK2(status >= 0, "Nap_AppendNAOValue: Error calling sprintf");
	...
	default:
	    ...
	    CHECK2(dblNao, "Nap_AppendNAOValue: Error calling Nap_CastNAO");
	    if (dblNao->rank == 0) {
    ...
    Tcl_Obj *CONST      objv[])
{
    ...
    CHECK2(objc >= 3  &&  objc <= 4, "Nap_Poly: wrong # arguments");
    CHECK2(naoPtr->rank == 2, "Nap_Poly: Array has rank other than 2");
    CHECK2(dataType == NAP_F32,
	    "Nap_Poly: Array has data-type other than float");
    tmpNAO = Nap_GetNaoFromObj(nap_cd, objv[2]);
    CHECK2(tmpNAO, "Nap_Poly: Error in <XY>");
    CHECK2(tmpNAO->rank <= 2, "Nap_Poly: <XY> has rank > 2");
    pxyNAO = Nap_CastNAO(nap_cd, tmpNAO, NAP_I32);
    CHECK2(pxyNAO, "Nap_Poly: error calling Nap_CastNAO");
    if (pxyNAO != tmpNAO) {
	Nap_FreeNAO(nap_cd, tmpNAO);
    }
    if (pxyNAO->rank == 2) {
	CHECK2(pxyNAO->shape[0] == 2, "Nap_Poly: # rows in <XY> is not 2");
	px = pxyNAO->data.I32;
	n = pxyNAO->shape[1];
	...
	if (tmpNAO) {
	    ...
	    CHECK2(pxNAO, "Nap_Poly: error calling Nap_CastNAO");
	    if (pxNAO != tmpNAO) {
		Nap_FreeNAO(nap_cd, tmpNAO);
	    }
	} else {
	...
	CHECK2(tmpNAO, "Nap_Poly: Error in fill-value argument");
	CHECK2(tmpNAO->nels == 1, "Nap_Poly: fill-value has nels > 1");
	valueNAO = Nap_CastNAO(nap_cd, tmpNAO, NAP_F32);
	CHECK2(valueNAO, "Nap_Poly: error calling Nap_CastNAO");
	if (valueNAO != tmpNAO) {
	    Nap_FreeNAO(nap_cd, tmpNAO);
	}
	...
	CHECK2(FALSE, "Nap_Poly: method not fill or draw");
    }
	...
	if (n > 0  &&  naoBoxed[0]) {
		...
		for (k = 0; k < subscriptNAO->nels; k++) {
		    ...
		    status = Nap_SetValueCopy(nap_cd, n-1, prodShape+1, naoBoxed+1, 
			    src, dst + s * j, nels, m);
		    CHECK2(status == TCL_OK, "Nap_SetValueCopy: Error calling Nap_SetValueCopy");
		}
	    }
    ...
    Tcl_Obj *CONST      objv[])
{
    ...
    dataType = naoPtr->dataType;
    CHECK2(objc >= 3  &&  objc <= 5, "Nap_SetValue: wrong # arguments");
    naoPtr->step = NAP_UNKNOWN;
    if (objc > 4) {
	if (Tcl_GetCharLength(objv[4]) > 0) {
	    ...
	    CHECK2(subscriptNAO, "Nap_SetValue: Illegal index");
	    Nap_IncrRefCount(nap_cd, subscriptNAO);
	}
    }
    shape[0] = naoPtr->rank;
    newSubscriptNAO = Nap_NewNAO(nap_cd, NAP_BOXED, 1, shape);
    CHECK2(newSubscriptNAO, "Nap_SetValue: Error calling Nap_NewNAO");
    Nap_IncrRefCount(nap_cd, newSubscriptNAO);
    if (subscriptNAO) {
	if (subscriptNAO->dataType == NAP_BOXED) {
	    CHECK2(subscriptNAO->nels == naoPtr->rank, "Nap_SetValue: nels(index) != rank(array)");
		...
		if (tmpNAO) {
		    CHECK2(tmpNAO->rank <= 1, "Nap_SetValue: subscript has rank > 1");
		    Nap_IncrRefCount(nap_cd, tmpNAO);
		}
	    }
	} else {
	    CHECK2(naoPtr->rank == 1  &&  subscriptNAO->rank <= 1,
		    "Nap_SetValue: Full subscripts not supported");
	...
	CHECK2(tmpNAO, "Nap_SetValue: error calling Nap_GetMissingValueNAO");
    } else {
	tmpNAO = Nap_GetNaoFromObj(nap_cd, objv[3]);
	CHECK2(tmpNAO, "Nap_SetValue: Error in fill-value argument");
    }
    Nap_IncrRefCount(nap_cd, tmpNAO);
    valueNAO = Nap_CastNAO(nap_cd, tmpNAO, dataType);
    CHECK2(valueNAO, "Nap_SetValue: error calling Nap_CastNAO");
    ...
    Nap_NAO		*naoPtr)
{
	...
	for (j = 0; j < naoPtr->rank; j++) {
		...
		CHECK3(coordVar, "Nap_OOC_set: Unable to evaluate NAP expression \"%s\"", str);
	...
	break;
    case 1:	/* ref. count */
	CHECK2(objc <= 5, "Nap_OOC_set: Too many arguments");
	if (objc == 3) {
	    ++naoPtr->count;
	} else {
	...
	break;
    case 3:  		/* format */
	CHECK2(objc >= 3, "Nap_OOC_set: wrong # arguments");
	FREE(naoPtr->format);
	if (objc == 3) {
	    naoPtr->format = NULL;
	} else {
	    ...
	    CHECK2(naoPtr->format, "Nap_OOC_set: Error calling Nap_StrDup");
	}
	break;
    case 4:  		/* label */
	CHECK2(objc >= 3, "Nap_OOC_set: wrong # arguments");
	...
	if (objc == 3) {
	    ...
	    CHECK2(naoPtr->label, "Nap_OOC_set: Error calling Nap_StrDup");
	}
	break;
    case 5:  		/* link */
	CHECK2(objc >= 3, "Nap_OOC_set: wrong # arguments");
	...
	if (objc > 3) {
		...
		CHECK2(mv_NAO, "Nap_OOC_set: Error calling Nap_CastNAO");
	...
	if (objc == 3) {
		...
		CHECK2(naoPtr->unit, "Nap_OOC_set: Error calling Nap_StrDup");
	    }
	}
	break;
    case 8:  		/* value */
	...
	break;
    default:
	CHECK2(FALSE, "Nap_OOC_set: Illegal set option");
    }
    return TCL_OK;
}
    ...
    char                *channelName)   /* Treat NULL as "stdout" */
{
    ...
    CHECK2(channel, "Nap_PutRaw: error calling Tcl_GetChannel");
	...
	CHECK2(status == TCL_OK, "Nap_PutRaw: error calling Nap_swap_bytes");
	status = Tcl_Write(channel, buffer, nbytes);
	CHECK2(status == nbytes, "Nap_PutRaw: error calling Tcl_Write");
	FREE(buffer);
    } else {
...
#line 1016 "/home/dav480/tcl/nap/generic/nap_ooc.c,m4"
    ...
    Tcl_Obj *CONST      objv[])
{
	...
	case 1:		/* binary (same as write) */
	    ...
	    break;
	case 2:	/* blt_vector */
	    CHECK2(FALSE, "Nap_ObjectCommand: Method 'blt_vector' is no longer available");
	    break;
	case 3:	/* coord. vars */
	...
	case 4:	/* count */
	    ...
	    CHECK2(status > 0, "Nap_ObjectCommand: error calling sprintf");
	    status = Nap_AppendLines(nap_cd, s);
	    break;
	case 5:	/* datatype */
	    str = Nap_DataTypeToText(naoPtr->dataType);
	    CHECK2(str, "Nap_ObjectCommand: error calling Nap_DataTypeToText");
	...
	case 8:  		/* fill */
	    ...
	    break;
	case 9:			/* format */
	    CHECK2(objc == 2, "Nap_ObjectCommand: wrong # arguments");
